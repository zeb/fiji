...
public class DoubleTsvdDCT_2D {
	...
	/**
	 * Constructs new DoubleTsvdDCT_2D.
	 * 
	 * @param imB
	 *            blurred image
	 * @param imPSF
	 ...
	 */
	public DoubleTsvdDCT_2D(ImagePlus imB, ImagePlus imPSF, ResizingType resizing, OutputType output, boolean showPadded) {
	...
	public ImagePlus deblur(double threshold) {
		...
		double alpha = gcvTsvdDCT_2D(S, B);
		IJ.showStatus("TSVD: deblurring");
		E1 = DoubleCommon_2D.createFilter_2D(S, alpha);
		PSF = B.copy();
		...
		if (threshold == -1) {
			if (isPadded) {
				DoubleCommon_2D.assignPixelsToProcessorPadded(ip, PSF, bRows, bCols, bRowsOff, bColsOff, cmY);
			} else {
				DoubleCommon_2D.assignPixelsToProcessor(ip, PSF, cmY);
			}
		} else {
			if (isPadded) {
				DoubleCommon_2D.assignPixelsToProcessorPadded(ip, PSF, bRows, bCols, bRowsOff, bColsOff, cmY, threshold);
			} else {
				DoubleCommon_2D.assignPixelsToProcessor(ip, PSF, cmY, threshold);
			}
		}
	...
	private double gcvTsvdDCT_2D(DoubleMatrix2D S, DoubleMatrix2D Bhat) {
