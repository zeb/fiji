...
public class FloatTsvdFFT_2D {
	...
	/**
	 * Constructs new FloatTsvdFFT_2D.
	 * 
	 * @param imB
	 *            blurred image
	 * @param imPSF
	 ...
	 */
	public FloatTsvdFFT_2D(ImagePlus imB, ImagePlus imPSF, ResizingType resizing, OutputType output, boolean showPadded) {
	...
	public ImagePlus deblur(float threshold) {
		...
		float alpha = gcvTsvdFFT_2D((FComplexMatrix2D) S, (FComplexMatrix2D) B);
		IJ.showStatus("TSVD: deblurring");
		FComplexMatrix2D Sfilt = FloatCommon_2D.createFilter_2D((FComplexMatrix2D) S, alpha);
		PSF = ((FComplexMatrix2D) B).copy();
		((FComplexMatrix2D) PSF).assign(Sfilt, FComplexFunctions.mult);
		((FComplexMatrix2D) PSF).ifft2(true);
		IJ.showStatus("TSVD: finalizing");
		FloatProcessor ip = new FloatProcessor(bCols, bRows);
		if (threshold == -1) {
			if (isPadded) {
				FloatCommon_2D.assignPixelsToProcessorPadded(ip, (FComplexMatrix2D) PSF, bRows, bCols, bRowsOff, bColsOff, cmY);
			} else {
				FloatCommon_2D.assignPixelsToProcessor(ip, (FComplexMatrix2D) PSF, cmY);
			}
		} else {
			if (isPadded) {
				FloatCommon_2D.assignPixelsToProcessorPadded(ip, (FComplexMatrix2D) PSF, bRows, bCols, bRowsOff, bColsOff, cmY, threshold);
			} else {
				FloatCommon_2D.assignPixelsToProcessor(ip, (FComplexMatrix2D) PSF, cmY, threshold);
			}
		}
	...
	private float gcvTsvdFFT_2D(FComplexMatrix2D S, FComplexMatrix2D Bhat) {
