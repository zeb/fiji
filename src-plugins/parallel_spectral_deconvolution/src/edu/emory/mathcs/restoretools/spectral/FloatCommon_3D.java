 ...
 * 
 */
public class FloatCommon_3D {
	
	private FloatCommon_3D() {}
	 ...
	 *            matrix
	 */
	public static void assignPixelsToMatrix_3D(final ImageStack stack, final FloatMatrix3D X) {
		int slices = X.slices();
				...
				futures[j] = ConcurrencyUtils.threadPool.submit(new Runnable() {
					public void run() {
						for (int s = startslice; s < stopslice; s++) {
							...
							FloatCommon_2D.assignPixelsToMatrix_2D(X.viewSlice(s), ip);
						}
					}
				});
			}
	...
	public static void assignPixelsToStack(final ImageStack stack, final FComplexMatrix3D X, final java.awt.image.ColorModel cmY) {
		...
		for (int s = 0; s < slices; s++) {
			...
			FloatCommon_2D.assignPixelsToProcessor(ip, X.viewSlice(s), cmY);
	...
	public static void assignPixelsToStack(final ImageStack stack, final FComplexMatrix3D X, final java.awt.image.ColorModel cmY, final float threshold) {
		...
		for (int s = 0; s < slices; s++) {
			FloatProcessor ip = new FloatProcessor(cols, rows);
			FloatCommon_2D.assignPixelsToProcessor(ip, X.viewSlice(s), cmY, threshold);
			stack.addSlice(null, ip, s);
		}
	}
	...
	public static void assignPixelsToStackPadded(final ImageStack stack, final FComplexMatrix3D X, final int slices, final int rows, final int cols, final int sOff, final int rOff, final int cOff, final java.awt.image.ColorModel cmY) {
		for (int s = 0; s < slices; s++) {
			FloatProcessor ip = new FloatProcessor(cols, rows);
			FloatCommon_2D.assignPixelsToProcessorPadded(ip, X.viewSlice(s + sOff), rows, cols, rOff, cOff, cmY);
			stack.addSlice(null, ip, s);
		}
	}
	...
	public static void assignPixelsToStackPadded(final ImageStack stack, final FComplexMatrix3D X, final int slices, final int rows, final int cols, final int sOff, final int rOff, final int cOff, final java.awt.image.ColorModel cmY, final float threshold) {
		for (int s = 0; s < slices; s++) {
			FloatProcessor ip = new FloatProcessor(cols, rows);
			FloatCommon_2D.assignPixelsToProcessorPadded(ip, X.viewSlice(s + sOff), rows, cols, rOff, cOff, cmY, threshold);
			stack.addSlice(null, ip, s);
		}
	}
	...
	/**
	 * Converts the image into the given output type.
	 * 
	 * @param image
	 *            image
	 * @param output
	 ...
	 */
	public static FloatMatrix3D circShift_3D(FloatMatrix3D PSF, int[] center) {
	 ...
	 */
	public static FComplexMatrix3D createFilter_3D(final FComplexMatrix3D S, final float alpha) {
	 ...
	 * @return filtered matrix
	 */
	public static FloatMatrix3D createFilter_3D(final FloatMatrix3D S, final float alpha) {
		final int slices = S.slices();
		final int rows = S.rows();
		final int cols = S.columns();
	 ...
	 * @return shifted matrix
	 */
	public static FloatMatrix3D dctShift_3D(FloatMatrix3D PSF, int[] center) {
	 ...
	 */
	public static FloatMatrix3D padPeriodic_3D(final FloatMatrix3D X, final int slicesPad, final int rowsPad, final int colsPad) {
		final int slices = X.slices();
		final int rows = X.rows();
		final int cols = X.columns();
	 ...
	 * @return padded matrix
	 */
	public static FloatMatrix3D padReflexive_3D(final FloatMatrix3D X, final int slicesPad, final int rowsPad, final int colsPad) {
		final int slices = X.slices();
		final int rows = X.rows();
		final int cols = X.columns();
		if ((slices == slicesPad) && (rows == rowsPad) && (cols == colsPad)) {
	 ...
	 * @return padded matrix
	 */
	public static FloatMatrix3D padZero_3D(FloatMatrix3D X, int slicesPad, int rowsPad, int colsPad) {
		int slices = X.slices();
		int rows = X.rows();
		int cols = X.columns();
		if ((slices == slicesPad) && (rows == rowsPad) && (cols == colsPad)) {
