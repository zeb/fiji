...
public class DoubleCommon_3D {
	 ...
	 */
	public static DComplexMatrix3D createFilter_3D(final DComplexMatrix3D S, final double alpha) {
		...
		if ((np > 1) && (S.size() >= ConcurrencyUtils.getThreadsBeginN_3D())) {
			...
			for (int j = 0; j < np; j++) {
				...
				futures[j] = ConcurrencyUtils.threadPool.submit(new Runnable() {
					public void run() {
						double elem[];
						int idx = startslice * 2 * rows * cols;
						for (int s = startslice; s < stopslice; s++) {
							for (int r = 0; r < rows; r++) {
								for (int c = 0; c < cols; c++) {
									elem = S.getQuick(s, r, c);
									if (DComplex.abs(elem) >= alpha) {
										elem = DComplex.inv(elem);
	 ...
	 * @return filtered matrix
	 */
	public static DoubleMatrix3D createFilter_3D(final DoubleMatrix3D S, final double alpha) {
		final int slices = S.slices();
		final int rows = S.rows();
		final int cols = S.columns();
	 ...
	 * @return shifted matrix
	 */
	public static DoubleMatrix3D dctShift_3D(DoubleMatrix3D PSF, int[] center) {
