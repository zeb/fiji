...
public class ICP < P extends Point & RealLocalizable >
{
	 ...
	 * Note that the elements of the {@link List}s have to implement {@link Point}(for compatibility with {@link Model}) and {@link RealLocalizable} for compatibility with {@link KDTree}).
	 *
	...
	public static ArrayList<PointMatch> removeAmbigousMatches( final List<PointMatch> matches )
	{
		...
		for ( int i = 0; i < matches.size(); i++ )
		{
			...
			if ( inconsistent.size() > 0 )
				for ( final int index : inconsistent )
		...
		if ( inconsistentCorrespondences.size() > 0 )
		{
			...
			for ( int i = inconsistentCorrespondences.size() - 1; i >= 0; i-- )
			{
				...
				final PointMatch pm = matches.get( inconsistentCorrespondences.get(i) );
	 ...
	 */
	protected static ArrayList<Integer> getOccurences( final Point pointTarget, final Point pointReference, final List<PointMatch> list )
	{
