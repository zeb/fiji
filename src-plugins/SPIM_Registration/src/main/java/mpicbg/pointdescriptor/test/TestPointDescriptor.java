...
public class TestPointDescriptor
{
	...
	protected void addAdvancedPoints( final ArrayList<Point> points1, final ArrayList<Point> points2 )
	{
		...
		final Random rnd = new Random(325235325L);

		for ( int i = 0; i < commonPoints; ++i )
		{
			// all between 5 and 10
			final float v1 = rnd.nextFloat()*5 + 5;
			final float v2 = rnd.nextFloat()*5 + 5;
			final float v3 = rnd.nextFloat()*5 + 5;
			final float o1 = (rnd.nextFloat()-0.5f)/10;
			final float o2 = (rnd.nextFloat()-0.5f)/10;
			final float o3 = (rnd.nextFloat()-0.5f)/10;
	                                                                                               ...
	                                                                                               final SimilarityMeasure similarityMeasure )
	{
		...
		final KNearestNeighborSearch< VirtualPointNode< P > > knnsearch = new KNearestNeighborSearchOnKDTree< VirtualPointNode< P > >( tree, k );
		final ArrayList< ModelPointDescriptor< P > > descriptors = new ArrayList< ModelPointDescriptor< P > > ( basisPoints.size() );
		...
		for ( final VirtualPointNode< P > p : basisPoints )
		{
			...
			for ( int i = 1; i < k; ++i )
				neighbors.add( knnsearch.getSampler( i ).get().getPoint() );
            ...
            final boolean normalize )
	{
		final int k = ( int ) Math.min( tree.size(), numNeighbors + 1 );
		final KNearestNeighborSearch< VirtualPointNode< P > > knnsearch = new KNearestNeighborSearchOnKDTree< VirtualPointNode< P > >( tree, k );
		final ArrayList< LocalCoordinateSystemPointDescriptor< P > > descriptors = new ArrayList< LocalCoordinateSystemPointDescriptor< P > > ( basisPoints.size() );
	...
	public static void testQuaternions()
	{
        ...
        qu.set( m );
        final Vector3f v1 = new Vector3f( qu.getX(), qu.getY(), qu.getZ() );
        v1.normalize();
        ...
        trans.get( m );
        qu.set( m );
        final Vector3f v2 = new Vector3f( qu.getX(), qu.getY(), qu.getZ() );
        v2.normalize();
        System.out.println( "Axis: " + v2  );
        System.out.println( Math.toDegrees( Math.acos( qu.getW() ) * 2 ) );

        final Vector3f v3 = new Vector3f( 1.1f, 0.1f, 0.25f );
        v3.normalize();

       	final Point3f p1 = new Point3f( 1, 0, 0 );
       	final Point3f p2 = new Point3f( v3 );
	...
	public static void testStability( final int numNeighbors, final int numTrueMatches, final int numRandomPoints, final double nTimesBetter, final float stdev, final boolean fastMatching, final boolean showPoints )
	{
		...
		final Point offset = new Point( new float[]{ -cubeTrueKantenLength/2, -cubeTrueKantenLength/2, -cubeTrueKantenLength/2 } );
		...
		if ( showPoints )
		{
			...
			final Color3f colorTrue = new Color3f( 38f/255f, 140f/255f, 0.1f );
			final Color3f colorFalse = new Color3f( 1f, 0.1f, 0.1f );
			final float size = .1f;

			final CustomLineMesh c = new CustomLineMesh( getBoundingBox( -cubeFalseKantenLength/2, cubeFalseKantenLength/2 ), CustomLineMesh.PAIRWISE );
			c.setLineWidth( 2 );
			c.setColor( colorFalse );
			final Content content = univ.addCustomMesh( c, "BoundingBoxFalse" );
			content.showCoordinateSystem(false);

			final CustomLineMesh c2 = new CustomLineMesh( getBoundingBox( -cubeTrueKantenLength/2, cubeTrueKantenLength/2 ), CustomLineMesh.PAIRWISE );
		...
		if ( fastMatching )
		{
			...
			for ( final LocalCoordinateSystemPointDescriptor< Point > descriptorA : descriptors1 )
			{
				...
				final double best = descriptorA.descriptorDistance( nnsearch.getSampler( 0 ).get() );
				final double secondBest = descriptorA.descriptorDistance( nnsearch.getSampler( 1 ).get() );
//				final double best = nnsearch.getSquareDistance( 0 );
//				final double secondBest = nnsearch.getSquareDistance( 1 );
				...
				if ( best * nTimesBetter < secondBest )
				{
					...
					{
						++detectedRight;
						final ArrayList< Point > neighbors = descriptorA.getOrderedNearestNeighboringPoints();
	...
	protected static boolean isCorrect( final Point a, final Point b )
	{
	...
	{
		final boolean showPoints = false;
