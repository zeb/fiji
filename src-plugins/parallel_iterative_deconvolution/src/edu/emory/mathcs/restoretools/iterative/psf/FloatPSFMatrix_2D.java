 ...
 * 
 */
public class FloatPSFMatrix_2D {
	...
	private FComplexMatrix2D[][] matdata;

	/**
	 * Creates new instance of FloatPSFMatrix_2D.
	 * 
	 * @param imPSF
	 *            array of PSF images
	 * @param boundary
	 ...
	 */
	public FloatPSFMatrix_2D(ImagePlus[][] imPSF, BoundaryType boundary, ResizingType resizing, int[] imSize) {
		PSF = new FloatPSF_2D(imPSF);
		...
		if (PSF.getNumberOfImages() > 1) {
			...
			varPadSize[0] = (int) Math.round((float) varPsfSize[0] / 2.0);
			varPadSize[1] = (int) Math.round((float) varPsfSize[1] / 2.0);
			constructMatrix();
			varNregions = new int[2];
			varNregions[0] = matdata.length;
			varNregions[1] = matdata[0].length;
			varRsize = new int[2];
			varRsize[0] = (int) Math.ceil((float) imSize[0] / (float) varNregions[0]);
			varRsize[1] = (int) Math.ceil((float) imSize[1] / (float) varNregions[1]);
	...
	/**
	 * Computes matrix-vector multiplication: A*b or A'*b, where A is this
	 * PSFMatrix_2D.
	 * 
	 * @param b
	 *            vector
	 ...
	 * @return A*B or A'*B
	 */
	public FloatMatrix2D times(FloatMatrix2D B, boolean transpose) {
		FloatMatrix2D Bpad = null;
		switch (type) {
		case INVARIANT:
			switch (boundary) {
			case ZERO:
				Bpad = FloatCommon_2D.padZero_2D(B, invPsfSize[0], invPsfSize[1]);
				break;
			case PERIODIC:
				Bpad = FloatCommon_2D.padPeriodic_2D(B, invPsfSize[0], invPsfSize[1]);
				break;
			case REFLEXIVE:
				Bpad = FloatCommon_2D.padReflexive_2D(B, invPsfSize[0], invPsfSize[1]);
				break;
			}
		...
		case VARIANT:
			switch (boundary) {
			case ZERO:
				Bpad = FloatCommon_2D.padZero_2D(B, imSize[0] + 2 * varPadSize[0], imSize[1] + 2 * varPadSize[1]);
				break;
			case REFLEXIVE:
				Bpad = FloatCommon_2D.padReflexive_2D(B, imSize[0] + 2 * varPadSize[0], imSize[1] + 2 * varPadSize[1]);
				break;
			case PERIODIC:
				Bpad = FloatCommon_2D.padPeriodic_2D(B, imSize[0] + 2 * varPadSize[0], imSize[1] + 2 * varPadSize[1]);
				break;
			}
			if (transpose) {
	...
	private FloatMatrix2D invariantMultiply(AbstractMatrix2D Bpad, boolean transpose) {
		Bpad = ((FloatMatrix2D) Bpad).getFft2();
		if (transpose) {
			((FComplexMatrix2D) Bpad).assign(matdata[0][0], FComplexFunctions.multConjSecond);
		} else {
			((FComplexMatrix2D) Bpad).assign(matdata[0][0], FComplexFunctions.mult);
		}
		((FComplexMatrix2D) Bpad).ifft2(true);
		Bpad = ((FComplexMatrix2D) Bpad).getRealPart();
		return ((FloatMatrix2D) Bpad).viewPart(invPadSize[0], invPadSize[1], imSize[0], imSize[1]);
	}

	private FloatMatrix2D variantMultiply(FloatMatrix2D Bpad) {
		if ((varPadSize1[0] > 0) || (varPadSize1[1] > 0)) {
			Bpad = FloatCommon_2D.padZero_2D(Bpad, varPadSize1, PaddingType.POST);
		}
		FloatMatrix2D Y = new DenseFloatMatrix2D(Bpad.rows(), Bpad.columns());
		for (int i = 0; i < varNregions[0]; i++) {
			for (int j = 0; j < varNregions[1]; j++) {
				FloatMatrix2D Xt = Bpad.viewPart(varEri.iidx[i][0], varEri.jidx[j][0], varEri.iidx[i][1] - varEri.iidx[i][0] + 1, varEri.jidx[j][1] - varEri.jidx[j][0] + 1);
				FloatMatrix2D Yt = variantMultiplyOnePsf(i, j, Xt, false);
	...
	private FloatMatrix2D variantTransposeMultiply(FloatMatrix2D Bpad) {
		if ((varPadSize1[0] > 0) || (varPadSize1[1] > 0)) {
			Bpad = FloatCommon_2D.padZero_2D(Bpad, varPadSize1, PaddingType.POST);
		}
		FloatMatrix2D Y = new DenseFloatMatrix2D(Bpad.rows(), Bpad.columns());
		...
		for (int i = 0; i < varNregions[0]; i++) {
			for (int j = 0; j < varNregions[1]; j++) {
				...
				FloatMatrix2D Xt = new DenseFloatMatrix2D(eregionSize[0], eregionSize[1]);
				...
				Xt = FloatCommon_2D.padZero_2D(Xt, varPadSize, PaddingType.BOTH);
				FloatMatrix2D Yt = variantMultiplyOnePsf(i, j, Xt, true);
				Y.viewPart(varEri.iidx[i][0], varEri.jidx[j][0], varEri.iidx[i][1] - varEri.iidx[i][0] + 1, varEri.jidx[j][1] - varEri.jidx[j][0] + 1).assign(
						Y.viewPart(varEri.iidx[i][0], varEri.jidx[j][0], varEri.iidx[i][1] - varEri.iidx[i][0] + 1, varEri.jidx[j][1] - varEri.jidx[j][0] + 1).assign(Yt, FloatFunctions.plus));
			}
		}
		int[][] idx = new int[2][2];
		...
		Y = Y.viewPart(idx[0][0], idx[0][1], idx[1][0] - idx[0][0], idx[1][1] - idx[0][1]);
		return Y;
	}

	private FloatMatrix2D variantMultiplyOnePsf(int row, int col, FloatMatrix2D X, boolean transpose) {
		...
		if ((padSize1[0] > 0) || padSize1[1] > 0) {
			X = FloatCommon_2D.padZero_2D(X, padSize1, PaddingType.POST);
		}
		...
		FloatMatrix2D Y = new DenseFloatMatrix2D(X.rows(), X.columns());
		for (int i = 0; i < pi.nregions[0]; i++) {
			for (int j = 0; j < pi.nregions[1]; j++) {
				FloatMatrix2D Xt = X.viewPart(eri.iidx[i][0], eri.jidx[j][0], eri.iidx[i][1] - eri.iidx[i][0] + 1, eri.jidx[j][1] - eri.jidx[j][0] + 1);
				FloatMatrix2D Yt = multiplyOneRegion(row, col, Xt, transpose);
	...
	private FloatMatrix2D multiplyOneRegion(int row, int col, AbstractMatrix2D X, boolean transpose) {
		int rowsX = X.rows();
		int colsX = X.columns();
		...
		if ((padSize[0] > 0) || padSize[1] > 0) {
			X = FloatCommon_2D.padZero_2D((FloatMatrix2D) X, padSize, PaddingType.POST);
		} else {
			if (X.isView()) {
				X = ((FloatMatrix2D) X).copy();
			}
		}
		X = ((FloatMatrix2D) X).getFft2();
		if (transpose) {
			((FComplexMatrix2D) X).assign(matdata[row][col], FComplexFunctions.multConjSecond);
		} else {
			((FComplexMatrix2D) X).assign(matdata[row][col], FComplexFunctions.mult);
		}
		((FComplexMatrix2D) X).ifft2(true);
		X = ((FComplexMatrix2D) X).getRealPart();
		X = ((FloatMatrix2D) X).viewPart(0, 0, rowsX, colsX);
		return (FloatMatrix2D) X;
	}
	...
	private RegionIndices eregionIndices(int[][] iidx, int[][] jidx, int[] nregions, int[] rsize) {
		...
		for (int i = 0; i < nregions[0]; i++) {
			...
			eri.iidx[i][1] = iidx[i][1] + 2 * (int) Math.floor((float) rsize[0] / 2.0);
		}
		for (int i = 0; i < nregions[1]; i++) {
			eri.jidx[i][0] = jidx[i][0];
			eri.jidx[i][1] = jidx[i][1] + 2 * (int) Math.floor((float) rsize[1] / 2.0);
		}
		return eri;
	}
	...
	private void constructMatrix() {
		FloatMatrix2D[][] image = PSF.getImage();
		int[][][] center = PSF.getCenter();
		switch (type) {
		case INVARIANT:
			matdata = new FComplexMatrix2D[1][1];
			matdata[0][0] = onePsfMatrixInvariant(image[0][0], center[0][0]);
			break;
		case VARIANT:
			matdata = new FComplexMatrix2D[image.length][image[0].length];
	...
	private FComplexMatrix2D onePsfMatrixInvariant(FloatMatrix2D image, int[] center) {
		image.normalize();
		int rows = image.rows();
		int columns = image.columns();
		AbstractMatrix2D matdata = new DenseFloatMatrix2D(invPsfSize[0], invPsfSize[1]);
		((FloatMatrix2D) matdata).viewPart(0, 0, rows, columns).assign(image);
		matdata = FloatCommon_2D.circShift_2D((FloatMatrix2D) matdata, center);
		matdata = ((FloatMatrix2D) matdata).getFft2();
		return (FComplexMatrix2D) matdata;
	}

	private FComplexMatrix2D onePsfMatrixVariant(FloatMatrix2D image, int[] center) {
		image.normalize();
		...
		AbstractMatrix2D matdata = new DenseFloatMatrix2D(rows, columns);
		((FloatMatrix2D) matdata).viewPart(0, 0, image.rows(), image.columns()).assign(image);
		matdata = FloatCommon_2D.circShift_2D((FloatMatrix2D) matdata, center);
		matdata = ((FloatMatrix2D) matdata).getFft2();
		return (FComplexMatrix2D) matdata;
	}
