...
import edu.emory.mathcs.restoretools.iterative.method.hybr.lbd.DoubleLBD_3D;
import edu.emory.mathcs.restoretools.iterative.method.hybr.lbd.DoublePLBD_3D;
import edu.emory.mathcs.restoretools.iterative.method.hybr.lbd.DoubleSimpleLBD_3D;
...
public class DoubleHyBR_3D {
	...
	/**
	 * Creates new instance of DoubleHyBR_3D
	 * 
	 * @param imB
	 *            blurred image
	 * @param imPSF
	 ...
	 */
	public DoubleHyBR_3D(ImagePlus imB, ImagePlus[][][] imPSF, BoundaryType boundary, ResizingType resizing, OutputType output, PreconditionerType Prec, double precTol, int maxIts, DoubleHyBROptions options, boolean showIteration) {
		...
		bsize = new int[] { bdepth, bheight, bwidth };
	...
	public ImagePlus deblur(double threshold) {
		...
		for (i = 0; i <= maxIts; i++) {
			...
			if (i >= 1) {
					...
					if ((i > 1) && (terminate == true)) {
						if (Math.abs((GCV.get(i) - GCV.get(i - 1))) / GCV.get(options.begReg) < options.flatTol) {
							...
							if (showIteration == false) {
								if (threshold == -1.0) {
									DoubleCommon_3D.assignPixelsToStack(is, x_out, bsize, cmY);
								} else {
									DoubleCommon_3D.assignPixelsToStack(is, x_out, bsize, cmY, threshold);
								}
								imX = new ImagePlus("(deblurred)", is);
							} else {
								if (threshold == -1.0) {
									DoubleCommon_3D.updatePixelsInStack(is, x_out, bsize, cmY);
								} else {
									DoubleCommon_3D.updatePixelsInStack(is, x_out, bsize, cmY, threshold);
								}
								imX.updateAndDraw();
							}
				...
				if (showIteration == true) {
					if (threshold == -1.0) {
						DoubleCommon_3D.updatePixelsInStack(is, x, bsize, cmY);
					} else {
						DoubleCommon_3D.updatePixelsInStack(is, x, bsize, cmY, threshold);
					}
					imX.updateAndDraw();
				}
			}
