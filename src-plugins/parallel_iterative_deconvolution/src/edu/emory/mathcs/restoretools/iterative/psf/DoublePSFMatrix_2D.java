 ...
 * 
 */
public class DoublePSFMatrix_2D {
	...
	/**
	 * Creates new instance of DoublePSFMatrix_2D.
	 * 
	 * @param imPSF
	 *            array of PSF images
	 * @param boundary
	 ...
	 */
	public DoublePSFMatrix_2D(ImagePlus[][] imPSF, BoundaryType boundary, ResizingType resizing, int[] imSize) {
		PSF = new DoublePSF_2D(imPSF);
	...
	public DoubleMatrix2D times(DoubleMatrix2D B, boolean transpose) {
		...
		case INVARIANT:
			switch (boundary) {
			case ZERO:
				Bpad = DoubleCommon_2D.padZero_2D(B, invPsfSize[0], invPsfSize[1]);
				break;
			case PERIODIC:
				Bpad = DoubleCommon_2D.padPeriodic_2D(B, invPsfSize[0], invPsfSize[1]);
				break;
			case REFLEXIVE:
				Bpad = DoubleCommon_2D.padReflexive_2D(B, invPsfSize[0], invPsfSize[1]);
				break;
			}
		...
		case VARIANT:
			switch (boundary) {
			case ZERO:
				Bpad = DoubleCommon_2D.padZero_2D(B, imSize[0] + 2 * varPadSize[0], imSize[1] + 2 * varPadSize[1]);
				break;
			case REFLEXIVE:
				Bpad = DoubleCommon_2D.padReflexive_2D(B, imSize[0] + 2 * varPadSize[0], imSize[1] + 2 * varPadSize[1]);
				break;
			case PERIODIC:
				Bpad = DoubleCommon_2D.padPeriodic_2D(B, imSize[0] + 2 * varPadSize[0], imSize[1] + 2 * varPadSize[1]);
				break;
			}
			if (transpose) {
	...
	private DoubleMatrix2D invariantMultiply(AbstractMatrix2D Bpad, boolean transpose) {
		Bpad = ((DoubleMatrix2D) Bpad).getFft2();
		if (transpose) {
		...
		((DComplexMatrix2D) Bpad).ifft2(true);
		Bpad = ((DComplexMatrix2D) Bpad).getRealPart();
		return ((DoubleMatrix2D) Bpad).viewPart(invPadSize[0], invPadSize[1], imSize[0], imSize[1]);
	}

	private DoubleMatrix2D variantMultiply(DoubleMatrix2D Bpad) {
		if ((varPadSize1[0] > 0) || (varPadSize1[1] > 0)) {
			Bpad = DoubleCommon_2D.padZero_2D(Bpad, varPadSize1, PaddingType.POST);
		}
	...
	private DoubleMatrix2D variantTransposeMultiply(DoubleMatrix2D Bpad) {
		...
		for (int i = 0; i < varNregions[0]; i++) {
			for (int j = 0; j < varNregions[1]; j++) {
				...
				Xt = DoubleCommon_2D.padZero_2D(Xt, varPadSize, PaddingType.BOTH);
	...
	private DoubleMatrix2D variantMultiplyOnePsf(int row, int col, DoubleMatrix2D X, boolean transpose) {
		...
		if ((padSize1[0] > 0) || padSize1[1] > 0) {
			X = DoubleCommon_2D.padZero_2D(X, padSize1, PaddingType.POST);
		}
	...
	private DoubleMatrix2D multiplyOneRegion(int row, int col, AbstractMatrix2D X, boolean transpose) {
		...
		if ((padSize[0] > 0) || padSize[1] > 0) {
			X = DoubleCommon_2D.padZero_2D((DoubleMatrix2D) X, padSize, PaddingType.POST);
		} else {
			if (X.isView()) {
				X = ((DoubleMatrix2D) X).copy();
			}
		}
		X = ((DoubleMatrix2D) X).getFft2();
		if (transpose) {
