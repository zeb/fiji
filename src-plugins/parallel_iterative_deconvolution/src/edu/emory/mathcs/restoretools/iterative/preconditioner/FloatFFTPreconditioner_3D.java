 ...
 * 
 */
public class FloatFFTPreconditioner_3D implements FloatPreconditioner_3D {
	...
	/**
	 * Creates new instance of FloatFFTPreconditioner_3D.
	 * 
	 * @param PSFMatrix
	 *            PSF matrix
	 * @param B
	 ...
	 *            tolerance
	 */
	public FloatFFTPreconditioner_3D(FloatPSFMatrix_3D PSFMatrix, FloatMatrix3D B, float tol) {
		this.tol = tol;
	...
	public FloatMatrix1D solve(FloatMatrix1D b, boolean transpose) {
		FloatMatrix3D B = b.reshape(imSize[0], imSize[1], imSize[2]);
		B = solve(B, transpose);
		return B.vectorize();
	}
	...
	public FloatMatrix3D solve(AbstractMatrix3D B, boolean transpose) {		
		switch (boundary) {
		case ZERO:
			B = FloatCommon_3D.padZero_3D((FloatMatrix3D) B, psfSize[0], psfSize[1], psfSize[2]);
			break;
		case PERIODIC:
			B = FloatCommon_3D.padPeriodic_3D((FloatMatrix3D) B, psfSize[0], psfSize[1], psfSize[2]);
			break;
		case REFLEXIVE:
			B = FloatCommon_3D.padReflexive_3D((FloatMatrix3D) B, psfSize[0], psfSize[1], psfSize[2]);
			break;
		}
		...
		((FComplexMatrix3D) B).ifft3(true);
		B = ((FComplexMatrix3D) B).getRealPart();
		B = ((FloatMatrix3D) B).viewPart(padSize[0], padSize[1], padSize[2], imSize[0], imSize[1], imSize[2]);
		return ((FloatMatrix3D) B);
	}
	...
	private void constructMatrix(FloatMatrix3D[][][] PSFs, FloatMatrix3D B, int[][][][] center) {
		...
		switch (boundary) {
		case ZERO:
			B = FloatCommon_3D.padZero_3D(B, psfSize[0], psfSize[1], psfSize[2]);
			break;
		case PERIODIC:
			B = FloatCommon_3D.padPeriodic_3D(B, psfSize[0], psfSize[1], psfSize[2]);
			break;
		case REFLEXIVE:
			B = FloatCommon_3D.padReflexive_3D(B, psfSize[0], psfSize[1], psfSize[2]);
			break;
		}
	...
	private void precMatrixOnePsf(int[] center, FloatMatrix3D Bpad) {
		...
		if ((padSize[0] > 0) || (padSize[1] > 0) || (padSize[2] > 0)) {
			matdata = FloatCommon_3D.padZero_3D((FloatMatrix3D) matdata, padSize, PaddingType.POST);
		}
		...
		final FloatMatrix3D Ef = (FloatMatrix3D) E;
							...
							for (int s = startslice; s < stopslice; s++) {
								for (int r = 0; r < rows; r++) {
									for (int c = 0; c < cols; c++) {
										elem = ((FComplexMatrix3D) matdata).getQuick(s, r, c);
										if (Ef.getQuick(s, r, c) >= tol) {
											((FComplexMatrix3D) matdata).setQuick(s, r, c, FComplex.mult(FComplex.inv(elem), maxE));
										} else {
											((FComplexMatrix3D) matdata).setQuick(s, r, c, one);
										}
									}
								}
							}
							...
							for (int s = startslice; s < stopslice; s++) {
								for (int r = 0; r < rows; r++) {
									for (int c = 0; c < cols; c++) {
										elem = ((FComplexMatrix3D) matdata).getQuick(s, r, c);
										if (Ef.getQuick(s, r, c) >= tol) {
											((FComplexMatrix3D) matdata).setQuick(s, r, c, FComplex.inv(elem));
										} else {
											((FComplexMatrix3D) matdata).setQuick(s, r, c, one);
										}
									}
	...
	private float defaultTol2(FloatMatrix3D E, FloatMatrix3D B) {
		...
		FloatMatrix1D bhat = new DenseFloatMatrix1D(Bhat.size(), (float[]) ((FloatMatrix3D) Bhat).elements(), 0, 1);
