...
import edu.emory.mathcs.restoretools.iterative.method.hybr.lbd.FloatLBD_3D;
import edu.emory.mathcs.restoretools.iterative.method.hybr.lbd.FloatPLBD_3D;
import edu.emory.mathcs.restoretools.iterative.method.hybr.lbd.FloatSimpleLBD_3D;
...
public class FloatHyBR_3D {
	...
	/**
	 * Creates new instance of FloatHyBR_3D
	 * 
	 * @param imB
	 *            blurred image
	 * @param imPSF
	 ...
	 */
	public FloatHyBR_3D(ImagePlus imB, ImagePlus[][][] imPSF, BoundaryType boundary, ResizingType resizing, OutputType output, PreconditionerType Prec, float precTol, int maxIts, FloatHyBROptions options, boolean showIteration) {
	...
	public ImagePlus deblur(float threshold) {
		...
		for (i = 0; i <= maxIts; i++) {
			...
			if (i >= 1) {
					...
					if ((i > 1) && (terminate == true)) {
						if (Math.abs((GCV.get(i) - GCV.get(i - 1))) / GCV.get(options.begReg) < options.flatTol) {
							...
							if (showIteration == false) {
								if (threshold == -1.0) {
									FloatCommon_3D.assignPixelsToStack(is, x_out, bsize, cmY);
								} else {
									FloatCommon_3D.assignPixelsToStack(is, x_out, bsize, cmY, threshold);
								}
								imX = new ImagePlus("(deblurred)", is);
							} else {
								if (threshold == -1.0) {
									FloatCommon_3D.updatePixelsInStack(is, x_out, bsize, cmY);
								} else {
									FloatCommon_3D.updatePixelsInStack(is, x_out, bsize, cmY, threshold);
								}
								imX.updateAndDraw();
							}
				...
				if (showIteration == true) {
					if (threshold == -1.0) {
						FloatCommon_3D.updatePixelsInStack(is, x, bsize, cmY);
					} else {
						FloatCommon_3D.updatePixelsInStack(is, x, bsize, cmY, threshold);
					}
					imX.updateAndDraw();
				}
			}
