 ...
 * 
 */
public class DoublePSFMatrix_3D {
	...
	/**
	 * Creates new instance of DoublePSFMatrix_3D.
	 * 
	 * @param imPSF
	 *            array of PSF images
	 * @param boundary
	 ...
	 */
	public DoublePSFMatrix_3D(ImagePlus[][][] imPSF, BoundaryType boundary, ResizingType resizing, int[] imSize) {
		PSF = new DoublePSF_3D(imPSF);
	...
	public DoubleMatrix3D times(DoubleMatrix3D B, boolean transpose) {
		...
		case INVARIANT:
			switch (boundary) {
			case ZERO:
				Bpad = DoubleCommon_3D.padZero_3D(B, invPsfSize[0], invPsfSize[1], invPsfSize[2]);
				break;
			case PERIODIC:
				Bpad = DoubleCommon_3D.padPeriodic_3D(B, invPsfSize[0], invPsfSize[1], invPsfSize[2]);
				break;
			case REFLEXIVE:
				Bpad = DoubleCommon_3D.padReflexive_3D(B, invPsfSize[0], invPsfSize[1], invPsfSize[2]);
				break;
			}
		...
		case VARIANT:
			switch (boundary) {
			case ZERO:
				Bpad = DoubleCommon_3D.padZero_3D(B, imSize[0] + 2 * varPadSize[0], imSize[1] + 2 * varPadSize[1], imSize[2] + 2 * varPadSize[2]);
				break;
			case REFLEXIVE:
				Bpad = DoubleCommon_3D.padReflexive_3D(B, imSize[0] + 2 * varPadSize[0], imSize[1] + 2 * varPadSize[1], imSize[2] + 2 * varPadSize[2]);
				break;
			case PERIODIC:
				Bpad = DoubleCommon_3D.padPeriodic_3D(B, imSize[0] + 2 * varPadSize[0], imSize[1] + 2 * varPadSize[1], imSize[2] + 2 * varPadSize[2]);
				break;
			}
			if (transpose) {
	...
	private DoubleMatrix3D invariantMultiply(AbstractMatrix3D Bpad, boolean transpose) {
		Bpad = ((DoubleMatrix3D) Bpad).getFft3();
		if (transpose) {
		...
		((DComplexMatrix3D) Bpad).ifft3(true);
		Bpad = ((DComplexMatrix3D) Bpad).getRealPart();
		return ((DoubleMatrix3D) Bpad).viewPart(invPadSize[0], invPadSize[1], invPadSize[2], imSize[0], imSize[1], imSize[2]);
	}

	private DoubleMatrix3D variantMultiply(DoubleMatrix3D Bpad) {
		if ((varPadSize1[0] > 0) || (varPadSize1[1] > 0)) {
			Bpad = DoubleCommon_3D.padZero_3D(Bpad, varPadSize1, PaddingType.POST);
		}
		...
		for (int i = 0; i < varNregions[0]; i++) {
			for (int j = 0; j < varNregions[1]; j++) {
				for (int k = 0; k < varNregions[2]; k++) {
					...
					Xt = DoubleCommon_3D.padZero_3D(Xt, varPadSize, PaddingType.BOTH);
	...
	private DoubleMatrix3D variantMultiplyOnePsf(int slice, int row, int col, DoubleMatrix3D X, boolean transpose) {
		...
		if ((padSize1[0] > 0) || padSize1[1] > 0 || padSize1[2] > 0) {
			X = DoubleCommon_3D.padZero_3D(X, padSize1, PaddingType.POST);
		}
	...
	private DoubleMatrix3D multiplyOneRegion(int slice, int row, int col, AbstractMatrix3D X, boolean transpose) {
		...
		if ((padSize[0] > 0) || padSize[1] > 0 || padSize[2] > 0) {
			X = DoubleCommon_3D.padZero_3D((DoubleMatrix3D) X, padSize, PaddingType.POST);
		} else {
			if (X.isView()) {
				X = ((DoubleMatrix3D) X).copy();
			}
		}
		X = ((DoubleMatrix3D) X).getFft3();
		if (transpose) {
		...
		((DComplexMatrix3D) X).ifft3(true);
		X = ((DComplexMatrix3D) X).getRealPart();
		X = ((DoubleMatrix3D) X).viewPart(0, 0, 0, slicesX, rowsX, colsX);
