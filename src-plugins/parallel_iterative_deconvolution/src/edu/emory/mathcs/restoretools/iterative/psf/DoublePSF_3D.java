 ...
 * 
 */
public class DoublePSF_3D {
	private DoubleMatrix3D[][][] image;

	private int[][][][] center;

	/**
	 * Creates new instance of DoublePSF_3D.
	 * 
	 * @param imPSF
	 *            array of PSF images
	 */
	public DoublePSF_3D(ImagePlus[][][] imPSF) {
		...
		for (int i = 0; i < imPSF.length; i++) {
			for (int j = 0; j < imPSF[0].length; j++) {
				for (int k = 0; k < imPSF[0][0].length; k++) {
					...
					image[i][j][k] = new DenseDoubleMatrix3D(isPSF.getSize(), isPSF.getHeight(), isPSF.getWidth());
					DoubleCommon_3D.assignPixelsToMatrix_3D(isPSF, image[i][j][k]);
