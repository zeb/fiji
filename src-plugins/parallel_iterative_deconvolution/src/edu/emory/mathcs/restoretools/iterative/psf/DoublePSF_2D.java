 ...
 * 
 */
public class DoublePSF_2D {
	private DoubleMatrix2D[][] image;

	private int[][][] center;

	/**
	 * Creates new instance of DoublePSF_2D.
	 * 
	 * @param imPSF
	 *            array of PSF images
	 */
	public DoublePSF_2D(ImagePlus[][] imPSF) {
		...
		for (int i = 0; i < imPSF.length; i++) {
			for (int j = 0; j < imPSF[0].length; j++) {
				...
				if (ipPSF instanceof FloatProcessor) {
					image[i][j].assign((float[]) ipPSF.getPixels());
				} else {
					image[i][j].assign((float[]) ipPSF.convertToFloat().getPixels());
				}
