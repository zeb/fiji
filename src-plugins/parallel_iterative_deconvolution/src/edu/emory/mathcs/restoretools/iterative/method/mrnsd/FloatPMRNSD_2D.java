...
public class FloatPMRNSD_2D {
	...
	/**
	 * Creates new instance of FloatPMRNSD_2D
	 * 
	 * @param imB
	 *            blurred image
	 * @param imPSF
	 ...
	 */
	public FloatPMRNSD_2D(ImagePlus imB, ImagePlus[][] imPSF, BoundaryType boundary, ResizingType resizing, OutputType output, PreconditionerType Prec, float precTol, int maxIts, float tol, boolean xnorm, boolean showIteration) {
	...
	public ImagePlus deblur(float threshold) {
		...
		if (minB < 0) {
			B.assign(FloatFunctions.minus(Math.min(0, minB) + sigsq));
		}
