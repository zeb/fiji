 ...
 * 
 */
public class FloatPSFMatrix_3D {
	...
	/**
	 * Creates new instance of FloatPSFMatrix_3D.
	 * 
	 * @param imPSF
	 *            array of PSF images
	 * @param boundary
	 ...
	 */
	public FloatPSFMatrix_3D(ImagePlus[][][] imPSF, BoundaryType boundary, ResizingType resizing, int[] imSize) {
		PSF = new FloatPSF_3D(imPSF);
		...
		if (PSF.getNumberOfImages() > 1) {
			...
			varPadSize[2] = (int) Math.round((float) varPsfSize[2] / 2.0);
			constructMatrix();
			...
			varRsize[2] = (int) Math.ceil((float) imSize[2] / (float) varNregions[2]);
	...
	public FloatMatrix1D times(FloatMatrix1D b, boolean transpose) {
		FloatMatrix3D B;
		B = b.reshape(imSize[0], imSize[1], imSize[2]);
		B = times(B, transpose);
		return B.vectorize();
	}
	...
	public FloatMatrix3D times(FloatMatrix3D B, boolean transpose) {
		...
		case INVARIANT:
			switch (boundary) {
			case ZERO:
				Bpad = FloatCommon_3D.padZero_3D(B, invPsfSize[0], invPsfSize[1], invPsfSize[2]);
				break;
			case PERIODIC:
				Bpad = FloatCommon_3D.padPeriodic_3D(B, invPsfSize[0], invPsfSize[1], invPsfSize[2]);
				break;
			case REFLEXIVE:
				Bpad = FloatCommon_3D.padReflexive_3D(B, invPsfSize[0], invPsfSize[1], invPsfSize[2]);
				break;
			}
		...
		case VARIANT:
			switch (boundary) {
			case ZERO:
				Bpad = FloatCommon_3D.padZero_3D(B, imSize[0] + 2 * varPadSize[0], imSize[1] + 2 * varPadSize[1], imSize[2] + 2 * varPadSize[2]);
				break;
			case REFLEXIVE:
				Bpad = FloatCommon_3D.padReflexive_3D(B, imSize[0] + 2 * varPadSize[0], imSize[1] + 2 * varPadSize[1], imSize[2] + 2 * varPadSize[2]);
				break;
			case PERIODIC:
				Bpad = FloatCommon_3D.padPeriodic_3D(B, imSize[0] + 2 * varPadSize[0], imSize[1] + 2 * varPadSize[1], imSize[2] + 2 * varPadSize[2]);
				break;
			}
			if (transpose) {
	...
	private FloatMatrix3D invariantMultiply(AbstractMatrix3D Bpad, boolean transpose) {
		Bpad = ((FloatMatrix3D) Bpad).getFft3();
		if (transpose) {
			((FComplexMatrix3D) Bpad).assign(matdata[0][0][0], FComplexFunctions.multConjSecond);
		} else {
			((FComplexMatrix3D) Bpad).assign(matdata[0][0][0], FComplexFunctions.mult);
		}
		((FComplexMatrix3D) Bpad).ifft3(true);
		Bpad = ((FComplexMatrix3D) Bpad).getRealPart();
		return ((FloatMatrix3D) Bpad).viewPart(invPadSize[0], invPadSize[1], invPadSize[2], imSize[0], imSize[1], imSize[2]);
	}

	private FloatMatrix3D variantMultiply(FloatMatrix3D Bpad) {
		if ((varPadSize1[0] > 0) || (varPadSize1[1] > 0)) {
			Bpad = FloatCommon_3D.padZero_3D(Bpad, varPadSize1, PaddingType.POST);
		}
		FloatMatrix3D Y = new DenseFloatMatrix3D(Bpad.slices(), Bpad.rows(), Bpad.columns());
		for (int i = 0; i < varNregions[0]; i++) {
			for (int j = 0; j < varNregions[1]; j++) {
				for (int k = 0; k < varNregions[2]; k++) {
					FloatMatrix3D Xt = Bpad.viewPart(varEri.iidx[i][0], varEri.jidx[j][0], varEri.kidx[k][0], varEri.iidx[i][1] - varEri.iidx[i][0] + 1, varEri.jidx[j][1] - varEri.jidx[j][0] + 1, varEri.kidx[k][1] - varEri.kidx[k][0] + 1);
					FloatMatrix3D Yt = variantMultiplyOnePsf(i, j, k, Xt, false);
	...
	private FloatMatrix3D variantTransposeMultiply(FloatMatrix3D Bpad) {
		...
		for (int i = 0; i < varNregions[0]; i++) {
			for (int j = 0; j < varNregions[1]; j++) {
				for (int k = 0; k < varNregions[2]; k++) {
					...
					FloatMatrix3D Xt = new DenseFloatMatrix3D(eregionSize[0], eregionSize[1], eregionSize[2]);
					...
					Xt = FloatCommon_3D.padZero_3D(Xt, varPadSize, PaddingType.BOTH);
					FloatMatrix3D Yt = variantMultiplyOnePsf(i, j, k, Xt, true);
					Y.viewPart(varEri.iidx[i][0], varEri.jidx[j][0], varEri.kidx[k][0], varEri.iidx[i][1] - varEri.iidx[i][0] + 1, varEri.jidx[j][1] - varEri.jidx[j][0] + 1, varEri.kidx[k][1] - varEri.kidx[k][0] + 1).assign(
							Y.viewPart(varEri.iidx[i][0], varEri.jidx[j][0], varEri.kidx[k][0], varEri.iidx[i][1] - varEri.iidx[i][0] + 1, varEri.jidx[j][1] - varEri.jidx[j][0] + 1, varEri.kidx[k][1] - varEri.kidx[k][0] + 1).assign(Yt, FloatFunctions.plus));
				}
			}
		}
		int[][] idx = new int[2][3];
		...
		Y = Y.viewPart(idx[0][0], idx[0][1], idx[0][2], idx[1][0] - idx[0][0], idx[1][1] - idx[0][1], idx[1][2] - idx[0][2]);
		return Y;
	}
	...
	private FloatMatrix3D variantMultiplyOnePsf(int slice, int row, int col, FloatMatrix3D X, boolean transpose) {
		...
		if ((padSize1[0] > 0) || padSize1[1] > 0 || padSize1[2] > 0) {
			X = FloatCommon_3D.padZero_3D(X, padSize1, PaddingType.POST);
		}
		...
		FloatMatrix3D Y = new DenseFloatMatrix3D(X.slices(), X.rows(), X.columns());
		for (int i = 0; i < pi.nregions[0]; i++) {
			for (int j = 0; j < pi.nregions[1]; j++) {
				for (int k = 0; k < pi.nregions[2]; k++) {
					...
					FloatMatrix3D Yt = multiplyOneRegion(slice, row, col, Xt, transpose);
	...
	private FloatMatrix3D multiplyOneRegion(int slice, int row, int col, AbstractMatrix3D X, boolean transpose) {
		...
		if ((padSize[0] > 0) || padSize[1] > 0 || padSize[2] > 0) {
			X = FloatCommon_3D.padZero_3D((FloatMatrix3D) X, padSize, PaddingType.POST);
		} else {
			if (X.isView()) {
				X = ((FloatMatrix3D) X).copy();
			}
		}
		X = ((FloatMatrix3D) X).getFft3();
		if (transpose) {
			((FComplexMatrix3D) X).assign(matdata[slice][row][col], FComplexFunctions.multConjSecond);
		} else {
			((FComplexMatrix3D) X).assign(matdata[slice][row][col], FComplexFunctions.mult);
		}
		((FComplexMatrix3D) X).ifft3(true);
		X = ((FComplexMatrix3D) X).getRealPart();
		X = ((FloatMatrix3D) X).viewPart(0, 0, 0, slicesX, rowsX, colsX);
	...
	private RegionIndices eregionIndices(int[][] iidx, int[][] jidx, int[][] kidx, int[] nregions, int[] rsize) {
		...
		for (int i = 0; i < nregions[2]; i++) {
			...
			eri.kidx[i][1] = kidx[i][1] + 2 * (int) Math.floor((float) rsize[2] / 2.0);
		}
		return eri;
	}
	...
	private void constructMatrix() {
		FloatMatrix3D[][][] image = PSF.getImage();
		int[][][][] center = PSF.getCenter();
		switch (type) {
		case INVARIANT:
			matdata = new FComplexMatrix3D[1][1][1];
			matdata[0][0][0] = onePsfMatrixInvariant(image[0][0][0], center[0][0][0]);
			break;
		case VARIANT:
			matdata = new FComplexMatrix3D[image.length][image[0].length][image[0][0].length];
	...
	private FComplexMatrix3D onePsfMatrixInvariant(FloatMatrix3D image, int[] center) {
		...
		AbstractMatrix3D matdata = new DenseFloatMatrix3D(invPsfSize[0], invPsfSize[1], invPsfSize[2]);
		((FloatMatrix3D) matdata).viewPart(0, 0, 0, slices, rows, columns).assign(image);
		matdata = FloatCommon_3D.circShift_3D((FloatMatrix3D) matdata, center);
		matdata = ((FloatMatrix3D) matdata).getFft3();
	...
	private FComplexMatrix3D onePsfMatrixVariant(FloatMatrix3D image, int[] center) {
		...
		AbstractMatrix3D matdata = new DenseFloatMatrix3D(slices, rows, columns);
