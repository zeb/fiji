...
/**
 * Common methods for 3D deblurring. Some code is from Bob Dougherty's Iterative
 * Deconvolve 3D.
 * 
 * @author Piotr Wendykier (piotr.wendykier@gmail.com)
 * 
 */
public class DoubleCommon_3D {
	
	private DoubleCommon_3D() {}
	 ...
	 *            matrix
	 */
	public static void assignPixelsToMatrix_3D(final ImageStack stack, final DoubleMatrix3D X) {
		int slices = X.slices();
				...
				futures[j] = ConcurrencyUtils.threadPool.submit(new Runnable() {
					public void run() {
						for (int s = startslice; s < stopslice; s++) {
							...
							DoubleCommon_2D.assignPixelsToMatrix_2D(X.viewSlice(s), ip);
						}
					}
				});
			}
	...
	public static void assignPixelsToStack(final ImageStack stack, final DoubleMatrix3D X, final java.awt.image.ColorModel cmY) {
		...
		for (int s = 0; s < slices; s++) {
			...
			DoubleCommon_2D.assignPixelsToProcessor(ip, X.viewSlice(s), cmY);
	 ...
	 * 
	 */
	public static void assignPixelsToStack(final ImageStack stack, final DoubleMatrix3D X, final java.awt.image.ColorModel cmY, double threshold) {
		final int slices = X.slices();
		final int rows = X.rows();
		final int cols = X.columns();
		for (int s = 0; s < slices; s++) {
			FloatProcessor ip = new FloatProcessor(cols, rows);
			DoubleCommon_2D.assignPixelsToProcessor(ip, X.viewSlice(s), cmY, threshold);
			stack.addSlice(null, ip, s);
		}
	}
	...
	public static void assignPixelsToStackPadded(final ImageStack stack, final DoubleMatrix3D X, final int slices, final int rows, final int cols, final int sOff, final int rOff, final int cOff, final java.awt.image.ColorModel cmY) {
		for (int s = 0; s < slices; s++) {
			FloatProcessor ip = new FloatProcessor(cols, rows);
			DoubleCommon_2D.assignPixelsToProcessorPadded(ip, X.viewSlice(s + sOff), rows, cols, rOff, cOff, cmY);
			stack.addSlice(null, ip, s);
		}
	}
	...
	public static void assignPixelsToStackPadded(final ImageStack stack, final DoubleMatrix3D X, final int slices, final int rows, final int cols, final int sOff, final int rOff, final int cOff, final java.awt.image.ColorModel cmY, final double threshold) {
		for (int s = 0; s < slices; s++) {
			FloatProcessor ip = new FloatProcessor(cols, rows);
			DoubleCommon_2D.assignPixelsToProcessorPadded(ip, X.viewSlice(s + sOff), rows, cols, rOff, cOff, cmY, threshold);
			stack.addSlice(null, ip, s);
		}
	}
	 ...
	 * 
	 */
	public static void updatePixelsInStack(final ImageStack stack, final DoubleMatrix3D X, final java.awt.image.ColorModel cmY) {
		final int slices = X.slices();
		for (int s = 0; s < slices; s++) {
	 ...
	 * 
	 */
	public static void updatePixelsInStackPadded(final ImageStack stack, final DoubleMatrix3D X, final int slices, final int rows, final int cols, final int sOff, final int rOff, final int cOff, final java.awt.image.ColorModel cmY, double threshold) {
		for (int s = 0; s < slices; s++) {
			FloatProcessor ip = (FloatProcessor) stack.getProcessor(s + 1);
			DoubleCommon_2D.assignPixelsToProcessorPadded(ip, X.viewSlice(s + sOff), rows, cols, rOff, cOff, cmY, threshold);
		}
	 ...
	 *            color model
	 */
	public static void updatePixelsInStackPadded(final ImageStack stack, final DoubleMatrix3D X, final int slices, final int rows, final int cols, final int sOff, final int rOff, final int cOff, final java.awt.image.ColorModel cmY) {
		for (int s = 0; s < slices; s++) {
			FloatProcessor ip = (FloatProcessor) stack.getProcessor(s + 1);
			DoubleCommon_2D.assignPixelsToProcessorPadded(ip, X.viewSlice(s + sOff), rows, cols, rOff, cOff, cmY);
		}
	 ...
	 * 
	 */
	public static void updatePixelsInStack(final ImageStack stack, final DoubleMatrix3D X, final java.awt.image.ColorModel cmY, double threshold) {
		final int slices = X.slices();
		for (int s = 0; s < slices; s++) {
			FloatProcessor ip = (FloatProcessor) stack.getProcessor(s + 1);
			DoubleCommon_2D.assignPixelsToProcessor(ip, X.viewSlice(s), cmY, threshold);
	 ...
	 * 
	 */
	public static void assignPixelsToStack(final ImageStack stack, final DoubleMatrix1D X, int[] xsize3D, final java.awt.image.ColorModel cmY) {
		...
		for (int s = 0; s < slices; s++) {
			FloatProcessor ip = new FloatProcessor(cols, rows);
			DoubleCommon_2D.assignPixelsToProcessor(ip, X.viewPart(s * sliceStride, sliceStride), cmY);
	 ...
	 * 
	 */
	public static void assignPixelsToStack(final ImageStack stack, final DoubleMatrix1D X, int[] xsize3D, final java.awt.image.ColorModel cmY, double threshold) {
		final int slices = xsize3D[0];
		final int rows = xsize3D[1];
		final int cols = xsize3D[2];
		final int sliceStride = rows * cols;
		...
		for (int s = 0; s < slices; s++) {
			FloatProcessor ip = new FloatProcessor(cols, rows);
			DoubleCommon_2D.assignPixelsToProcessor(ip, X.viewPart(s * sliceStride, sliceStride), cmY, threshold);
			ip.setColorModel(cmY);
			ip.setMinAndMax(0, 0);
			stack.addSlice(null, ip, s);
		}
	 ...
	 *            color model
	 */
	public static void updatePixelsInStack(final ImageStack stack, final DoubleMatrix1D X, int[] xsize3D, final java.awt.image.ColorModel cmY) {
		final int slices = xsize3D[0];
		final int rows = xsize3D[1];
		final int cols = xsize3D[2];
		final int sliceStride = rows * cols;
	 ...
	 * 
	 */
	public static void updatePixelsInStack(final ImageStack stack, final DoubleMatrix1D X, int[] xsize3D, final java.awt.image.ColorModel cmY, double threshold) {
		final int slices = xsize3D[0];
		final int rows = xsize3D[1];
		final int cols = xsize3D[2];
		final int sliceStride = rows * cols;
	 ...
	 */
	public static DoubleMatrix3D circShift_3D(DoubleMatrix3D PSF, int[] center) {
	 ...
	 */
	public static DoubleMatrix3D padPeriodic_3D(final DoubleMatrix3D X, final int slicesPad, final int rowsPad, final int colsPad) {
		final int slices = X.slices();
		final int rows = X.rows();
		final int cols = X.columns();
	 ...
	 * @return padded matrix
	 */
	public static DoubleMatrix3D padReflexive_3D(final DoubleMatrix3D X, final int slicesPad, final int rowsPad, final int colsPad) {
		final int slices = X.slices();
		final int rows = X.rows();
		final int cols = X.columns();
		if ((slices == slicesPad) && (rows == rowsPad) && (cols == colsPad)) {
	 ...
	 * @return padded matrix
	 */
	public static DoubleMatrix3D padZero_3D(DoubleMatrix3D X, int slicesPad, int rowsPad, int colsPad) {
		int slices = X.slices();
		int rows = X.rows();
		int cols = X.columns();
		if ((slices == slicesPad) && (rows == rowsPad) && (cols == colsPad)) {
	 ...
	 * @return padded matrix
	 */
	public static DoubleMatrix3D padZero_3D(DoubleMatrix3D X, int[] padSize, PaddingType padding) {
