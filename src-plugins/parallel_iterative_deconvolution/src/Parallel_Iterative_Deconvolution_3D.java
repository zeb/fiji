...
/**
 * Parallel Iterative Deconvolution 3D GUI
 * 
 * @author Piotr Wendykier (piotr.wendykier@gmail.com)
 * 
 */
public class Parallel_Iterative_Deconvolution_3D implements PlugIn, ImageListener {
	 ...
	 */
	public static String deconvolveWPL(String pathToBlurredImage, String pathToPsf, String pathToDeblurredImage, String boundaryStr, String resizingStr, String outputStr, String precisionStr, String thresholdStr, String nOfItersStr, String nOfThreadsStr, String showIterStr, String gammaStr,
		...
		switch (precision) {
		case DOUBLE:
			DoubleWPLOptions doptions = new DoubleWPLOptions(gamma, filterXY, filterXY, filterZ, normalize, logMean, antiRing, changeThreshPercent, db, detectDivergence);
			DoubleWPL_3D dwpl = new DoubleWPL_3D(imB, imPSF, boundary, resizing, output, nOfIters, doptions, showIteration);
			imX = dwpl.deblur(threshold);
			break;
		case SINGLE:
			FloatWPLOptions foptions = new FloatWPLOptions((float) gamma, (float) filterXY, (float) filterXY, (float) filterZ, normalize, logMean, antiRing, (float) changeThreshPercent, db, detectDivergence);
			FloatWPL_3D fwpl = new FloatWPL_3D(imB, imPSF, boundary, resizing, output, nOfIters, foptions, showIteration);
	...
	public static String deconvolveMRNSD(String pathToBlurredImage, String pathToPsf, String pathToDeblurredImage, String preconditionerStr, String preconditionerTolStr, String boundaryStr, String resizingStr, String outputStr, String precisionStr, String stoppingTolStr, String thresholdStr,
			String nOfItersStr, String nOfThreadsStr, String showIterStr) {
		double stoppingTol;
		...
		imPSF[0][0][0] = IJ.openImage(pathToPsf);
		if (imPSF[0][0][0] == null) {
			return "Cannot open image " + pathToPsf;
		}
		ImageProcessor ipB = imB.getProcessor();
		...
		case DOUBLE:
			if (preconditioner == PreconditionerType.NONE) {
				DoubleMRNSD_3D dmrnsd = new DoubleMRNSD_3D(imB, imPSF, boundary, resizing, output, nOfIters, stoppingTol, false, showIteration);
				imX = dmrnsd.deblur(threshold);
			} else {
				DoublePMRNSD_3D dpmrnsd = new DoublePMRNSD_3D(imB, imPSF, boundary, resizing, output, preconditioner, preconditionerTol, nOfIters, stoppingTol, false, showIteration);
				imX = dpmrnsd.deblur(threshold);
			}
			break;
		case SINGLE:
			if (preconditioner == PreconditionerType.NONE) {
				FloatMRNSD_3D fmrnsd = new FloatMRNSD_3D(imB, imPSF, boundary, resizing, output, nOfIters, (float) stoppingTol, false, showIteration);
				imX = fmrnsd.deblur((float) threshold);
			} else {
				FloatPMRNSD_3D fpmrnsd = new FloatPMRNSD_3D(imB, imPSF, boundary, resizing, output, preconditioner, (float) preconditionerTol, nOfIters, (float) stoppingTol, false, showIteration);
	 ...
	 * @return path to deblurred image or error message
	 */
	public static String deconvolveHyBR(String pathToBlurredImage, String pathToPsf, String pathToDeblurredImage, String preconditionerStr, String preconditionerTolStr, String boundaryStr, String resizingStr, String outputStr, String precisionStr, String thresholdStr, String nOfItersStr,
			String nOfThreadsStr, String showIterStr, String inSolvStr, String regMethodStr, String regParamStr, String omegaStr, String reorthStr, String begRegStr, String flatTolStr) {
		...
		case DOUBLE:
			...
			DoubleHyBR_3D dhybr = new DoubleHyBR_3D(imB, imPSF, boundary, resizing, output, preconditioner, preconditionerTol, nOfIters, doptions, showIteration);
		...
		case SINGLE:
			...
			FloatHyBR_3D fhybr = new FloatHyBR_3D(imB, imPSF, boundary, resizing, output, preconditioner, (float) preconditionerTol, nOfIters, foptions, showIteration);
	...
	public static String deconvolveCGLS(String pathToBlurredImage, String pathToPsf, String pathToDeblurredImage, String preconditionerStr, String preconditionerTolStr, String boundaryStr, String resizingStr, String outputStr, String precisionStr, String stoppingTolStr, String thresholdStr,
		...
		case DOUBLE:
			if (preconditioner == PreconditionerType.NONE) {
				DoubleCGLS_3D dcgls = new DoubleCGLS_3D(imB, imPSF, boundary, resizing, output, nOfIters, stoppingTol, false, showIteration);
				imX = dcgls.deblur(threshold);
			} else {
				DoublePCGLS_3D dpcgls = new DoublePCGLS_3D(imB, imPSF, boundary, resizing, output, preconditioner, preconditionerTol, nOfIters, stoppingTol, false, showIteration);
				imX = dpcgls.deblur(threshold);
			}
			break;
		case SINGLE:
			if (preconditioner == PreconditionerType.NONE) {
				FloatCGLS_3D fcgls = new FloatCGLS_3D(imB, imPSF, boundary, resizing, output, nOfIters, (float) stoppingTol, false, showIteration);
				imX = fcgls.deblur((float) threshold);
			} else {
				FloatPCGLS_3D fpcgls = new FloatPCGLS_3D(imB, imPSF, boundary, resizing, output, preconditioner, (float) preconditionerTol, nOfIters, (float) stoppingTol, false, showIteration);
	...
	private JFrame mainPanelFrame, PSFCreateFrame, PSFEditFrame, CGLSOptionsFrame, MRNSDOptionsFrame, HyBROptionsFrame, WPLOptionsFrame;

	private DoubleCGLS_3D dcgls;

	private DoublePCGLS_3D dpcgls;

	private DoubleHyBR_3D dhybr;

	private DoubleMRNSD_3D dmrnsd;

	private DoublePMRNSD_3D dpmrnsd;

	private DoubleWPL_3D dwpl;

	private FloatCGLS_3D fcgls;

	private FloatPCGLS_3D fpcgls;

	private FloatHyBR_3D fhybr;

	private FloatMRNSD_3D fmrnsd;

	private FloatPMRNSD_3D fpmrnsd;

	private FloatWPL_3D fwpl;
	...
	private int psfSlices, psfRows, psfColumns;
	...
	public void run(String arg) {
		...
		panel.init();
		mainPanelFrame = new JFrame("Parallel Iterative Deconvolution 3D " + version + " ");
	...
	private class WPLOptionsPanel extends JPanel {
		...
		private JTextField gammaField, filterXYfield, filterZfield, changeThreshPercentField, thresholdField;

		private JCheckBox normalizeCheck, logMeanCheck, antiRingCheck, dbCheck, detectDivergenceCheck, thresholdCheck;

		private double filterXY, filterZ, gamma, changeThreshPercent;
		...
		private void init() {
			...
			filterZPanel.setLayout(new FlowLayout(FlowLayout.LEADING));
			...
			filterZfield = new JTextField("1", 6);
			filterZfield.addActionListener(new FilterZFieldActionListener());
			filterZPanel.add(filterZfield);
			add(filterZPanel);
		...
		private class FilterZFieldActionListener implements ActionListener {
			public void actionPerformed(ActionEvent e) {
				try {
					filterZ = Double.parseDouble(filterZfield.getText());
				} catch (Exception ex) {
					IJ.error("Filter z must be a nonnegative value.");
				}
				if (filterZ < 0.0) {
					IJ.error("Filter z must be a nonnegative value.");
				}
	...
	private class PSFCreatePanel extends JPanel {
		...
		public JTextField slicesField, rowsField, columnsField;
		...
		private boolean checkTextFields() {
			...
			try {
				psfSlices = Integer.parseInt(slicesField.getText());
			} catch (Exception ex) {
				IJ.error("Number of slices must be a positive integer number.");
				return false;
			}
			if (psfSlices < 1) {
				IJ.error("Number of slices must be a positive integer number.");
			...
			if (moreThanOne == false) {
				IJ.error("The size of at least one dimension must be greater than 1");
				return false;
			}
	...
	private class PSFEditPanel extends JPanel {
		...
		private void init() {
			...
			tablePanels = new JPanel[psfSlices];
			cards = new JPanel(new CardLayout());
			counter = psfRows * psfColumns;
			buttons = new JButton[psfSlices][psfRows][psfColumns];
			for (int s = 0; s < psfSlices; s++) {
				tablePanels[s] = new JPanel();
				tablePanels[s].setLayout(new GridLayout(psfRows, psfColumns));
				for (int r = 0; r < psfRows; r++) {
					for (int c = 0; c < psfColumns; c++) {
						buttons[s][r][c] = new JButton("Null");
						buttons[s][r][c].setToolTipText("Null");
						buttons[s][r][c].addActionListener(new PSFButtonsActionListener());
						buttons[s][r][c].setHorizontalAlignment(SwingConstants.LEFT);
						tablePanels[s].add(buttons[s][r][c]);
					}
				}
				cards.add(tablePanels[s], new Integer(s).toString());
			}
			cl = (CardLayout) (cards.getLayout());
			cl.show(cards, new Integer(0).toString());
			currentTablePanel = 0;
			...
			if (psfSlices == 1) {
				...
				nextButton.addActionListener(new FinishButtonActionListener());
		...
		private class NextButtonActionListener implements ActionListener {
			public void actionPerformed(ActionEvent e) {
				currentTablePanel++;
				cl.show(cards, new Integer(currentTablePanel).toString());
				previousButton.setEnabled(true);
				if (currentTablePanel == psfSlices - 1) {
		...
		private class PreviousButtonActionListener implements ActionListener {
			public void actionPerformed(ActionEvent e) {
				currentTablePanel--;
				cl.show(cards, new Integer(currentTablePanel).toString());
				if (currentTablePanel == 0) {
					previousButton.setEnabled(false);
				}
				if (currentTablePanel == psfSlices - 2) {
					nextButton.removeActionListener(nextButton.getActionListeners()[0]);
					nextButton.addActionListener(new NextButtonActionListener());
		...
		private class FinishButtonActionListener implements ActionListener {
			public void actionPerformed(ActionEvent e) {
				imPSF = new ImagePlus[psfSlices][psfRows][psfColumns];
				Opener o = new Opener();
				for (int s = 0; s < psfSlices; s++) {
					for (int r = 0; r < psfRows; r++) {
						for (int c = 0; c < psfColumns; c++) {
							imPSF[s][r][c] = o.openImage(buttons[s][r][c].getText());
							ImageProcessor ipPSF = imPSF[s][r][c].getProcessor();
							if (ipPSF instanceof ColorProcessor) {
								IJ.showMessage("RGB images are not currently supported.");
								return;
							}
							if (imPSF[s][r][c].getStackSize() == 1) {
								IJ.showMessage("For 2D images use Parallel Iterative Deconvolution 2D");
								return;
							}
						}
					}
				...
				Thread thread = new Thread(new Runnable() {
					public void run() {
						...
						if (variantPSFCheck.isSelected() == false) {
							imPSF = new ImagePlus[1][1][1];
							imPSF[0][0][0] = WindowManager.getImage((String) psfChoice.getSelectedItem());
							ImageProcessor ipPSF = imPSF[0][0][0].getProcessor();
							if (ipPSF instanceof ColorProcessor) {
								IJ.showMessage("RGB images are not currently supported.");
								return;
						...
						case DOUBLE:
							...
							case CGLS:
								if (selPrecond == PreconditionerType.NONE) {
									dcgls = new DoubleCGLS_3D(imB, imPSF, selBoundary, selResizing, selOutput, iters, cglsStoppingTol, false, itersCheck.isSelected());
									imX = dcgls.deblur(cglsThreshold);
								} else {
									dpcgls = new DoublePCGLS_3D(imB, imPSF, selBoundary, selResizing, selOutput, selPrecond, precTol, iters, cglsStoppingTol, false, itersCheck.isSelected());
							...
							case MRNSD:
								if (selPrecond == PreconditionerType.NONE) {
									dmrnsd = new DoubleMRNSD_3D(imB, imPSF, selBoundary, selResizing, selOutput, iters, mrnsdStoppingTol, false, itersCheck.isSelected());
									imX = dmrnsd.deblur(mrnsdThreshold);
								} else {
									dpmrnsd = new DoublePMRNSD_3D(imB, imPSF, selBoundary, selResizing, selOutput, selPrecond, precTol, iters, mrnsdStoppingTol, false, itersCheck.isSelected());
							...
							case HyBR:
								if (selPrecond == PreconditionerType.NONE) {
									dhybr = new DoubleHyBR_3D(imB, imPSF, selBoundary, selResizing, selOutput, PreconditionerType.NONE, -1, iters, dHyBROptions, itersCheck.isSelected());
									imX = dhybr.deblur(hybrThreshold);
								} else {
									dhybr = new DoubleHyBR_3D(imB, imPSF, selBoundary, selResizing, selOutput, selPrecond, precTol, iters, dHyBROptions, itersCheck.isSelected());
								...
								break;
							case WPL:
								dwpl = new DoubleWPL_3D(imB, imPSF[0][0][0], selBoundary, selResizing, selOutput, iters, dWPLOptions, itersCheck.isSelected());
						...
						case SINGLE:
							...
							case CGLS:
								if (selPrecond == PreconditionerType.NONE) {
									fcgls = new FloatCGLS_3D(imB, imPSF, selBoundary, selResizing, selOutput, iters, (float) cglsStoppingTol, false, itersCheck.isSelected());
									imX = fcgls.deblur((float) cglsThreshold);
								} else {
									fpcgls = new FloatPCGLS_3D(imB, imPSF, selBoundary, selResizing, selOutput, selPrecond, (float) precTol, iters, (float) cglsStoppingTol, false, itersCheck.isSelected());
							...
							case MRNSD:
								if (selPrecond == PreconditionerType.NONE) {
									fmrnsd = new FloatMRNSD_3D(imB, imPSF, selBoundary, selResizing, selOutput, iters, (float) mrnsdStoppingTol, false, itersCheck.isSelected());
									imX = fmrnsd.deblur((float) mrnsdThreshold);
								} else {
									fpmrnsd = new FloatPMRNSD_3D(imB, imPSF, selBoundary, selResizing, selOutput, selPrecond, (float) precTol, iters, (float) mrnsdStoppingTol, false, itersCheck.isSelected());
							...
							case HyBR:
								if (selPrecond == PreconditionerType.NONE) {
									fhybr = new FloatHyBR_3D(imB, imPSF, selBoundary, selResizing, selOutput, PreconditionerType.NONE, -1, iters, fHyBROptions, itersCheck.isSelected());
									imX = fhybr.deblur((float) hybrThreshold);
								} else {
									fhybr = new FloatHyBR_3D(imB, imPSF, selBoundary, selResizing, selOutput, selPrecond, (float) precTol, iters, fHyBROptions, itersCheck.isSelected());
								...
								break;
							case WPL:
								fwpl = new FloatWPL_3D(imB, imPSF[0][0][0], selBoundary, selResizing, selOutput, iters, fWPLOptions, itersCheck.isSelected());
	...
	public static void main(String args[]) {
		...
		IJ.runPlugIn("Parallel_Iterative_Deconvolution_3D", null);
	}
}
