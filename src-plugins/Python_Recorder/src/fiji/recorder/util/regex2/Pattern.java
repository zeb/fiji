    ...
    implements java.io.Serializable
{
    ...
    private void normalize() {
        ...
        normalizedPattern = pattern; // Normalizer.normalize(pattern, Normalizer.Form.NFD);
...
loop:   for(int x=0, offset=0; x<nCodePoints; x++, offset+=len) {
    ...
    private int getClass(int c) {
        return c; //Normalizer.getCombiningClass(c);
    }
...
//        String result = Normalizer.normalize(firstTwoCharacters, Normalizer.Form.NFC);
        String result  = firstTwoCharacters;
...
//        }
    ...
    patternLength = j;
    /*
     * Next line is valid for java 1.6. Since we want to be 1,5 compatible, we have to do it by hand.
     */
    // temp = Arrays.copyOf(newtemp, j + 2); // double zero termination
    temp = new int[j+2];
    for (i=0; i<Math.min(temp.length, newtemp.length); i++) {     	temp[i] = newtemp[i]; } ;
        ...
        boolean match1(Matcher matcher, int i, int j, CharSequence seq) {
            for (;;) {
                ...
                if (capture) {
                    ...
                    matcher.groupNames[groupIndex/2] = matcher.name(0);
                }
                i = matcher.last;
                j++;
            }
    ...
    static final class GroupTail extends Node {
        ...
        boolean match(Matcher matcher, int i, CharSequence seq) {
            ...
            if (tmp >= 0) { // This is the normal group case.
                ...
                matcher.groupNames[groupIndex / 2] = name;
