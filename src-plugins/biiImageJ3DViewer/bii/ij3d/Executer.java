...
import bii.ij3d.shapes.Scalebar;
...
import bii.view4d.Viewer4D;
import bii.view4d.Viewer4DController;

import bii.math3d.Transform_IO;
...
import bii.vib.InterpolatedImage;
import bii.vib.FastMatrix;

import bii.orthoslice.OrthoGroup;
import bii.voltex.VoltexGroup;
import bii.voltex.VolumeRenderer;
import bii.isosurface.MeshExporter;
import bii.isosurface.MeshEditor;
...
import bii.octree.OctreeDialog;
...
public class Executer {
	...
	public void changeTransparency(final Content c) {
		...
		final GenericDialog gd = new BiiGenericDialogForTransparency(
			"Adjust transparency ...", transp_adjuster, c, univ);
		//final int oldTr = (int)(c.getTransparency() * 100);
		String oldTrStr = Prefs.get( "segmentingassistant.transparency", null );
		if (oldTrStr == null) oldTrStr = "35";
		final int oldTr = Integer.parseInt(oldTrStr);
		...
		((Scrollbar)gd.getSliders().get(0)).
			...
			public void adjustmentValueChanged(AdjustmentEvent e) {
				...
				Prefs.set("segmentingassistant.transparency", (int)e.getValue());
			}
		});
		/*
	...
	public void changeThreshold(final Content c) {
		...
		final GenericDialog gd = 
				new BiiGenericDialog("Adjust threshold...", thresh_adjuster, c, univ);
			...
			public void windowClosed(WindowEvent e) {
				try {
					if(gd.wasCanceled()) {
						...
						univ.fireContentChanged(c);
						BiiImageJ3DViewer.setCancelled();
						return;
					} else {
						record(SET_THRESHOLD, 
							Integer.toString(
								c.threshold));
						BiiImageJ3DViewer.setThreshold(Integer.toString(c.threshold));
					}
				} finally {
		...
		Point location = univ.getWindow().getLocation();
		int width = univ.getWindow().getWidth();
		gd.setLocation(location.x + width, location.y);
	}
	 ...
	 * *********************************************************/
	abstract class SliderAdjuster extends Thread {
		boolean go = false;
		int newV;
		Content content;
