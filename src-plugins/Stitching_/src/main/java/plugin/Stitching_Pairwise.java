...
public class Stitching_Pairwise implements PlugIn 
{
	...
	protected static < T extends RealType< T > & NativeType< T > > ImagePlus fuse( final T targetType, final ImagePlus imp1, final ImagePlus imp2, final ArrayList<InvertibleBoundable> models, final StitchingParameters params )
	{
		...
		else if ( params.fusionMethod == 5 ) // overlay
		{
			...
			final CompositeImage timepoint0 = OverlayFusion.createOverlay( targetType, images, models, params.dimensionality, 1, factory, outofboundsFactory );
			...
			if ( imp1.getNFrames() > 1 )
			{
				...
				{
					final CompositeImage tmp = OverlayFusion.createOverlay( targetType, images, models, params.dimensionality, f, factory, outofboundsFactory );
