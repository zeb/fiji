 ...
 */
public class MinMaxMedianPoint {
	...
	/** Constant representing the maximum filter returning a single point. */
	public static final int MAXIMUM_POINT = 3;
	...
	public static ImagePlus convolve(ImagePlus image, int method) {
		...
		if(method < 0 || method >= 4) {
			IJ.error("Neither MINIMUM nor MAXIMUM nor MEDIAN nor MAXIMUM_POINT chosen");
			return null;
		}
	...
	private static class FloatSlide extends Slide {
		...
		Float convolvePoint(int z, int y, int x, int method) {
			...
			if (method == MAXIMUM_POINT)
				if (values[size-1]<=(Float)slices_in[z].getValue(x,y))
					return values[size-1];
				else
					return (float)0.0;
			if(method == MINIMUM)
	...
	private static class ByteSlide extends Slide {
		...
		Byte convolvePoint(int z, int y, int x, int method) {
			...
			if (method == MAXIMUM_POINT){
				int tmp = (Byte)slices_in[z].getValue(x,y)&0xff;
				if (values[size-1]<=tmp)
					return (byte)values[size-1];
				else
					return 0;
